# AI Scene Director 1.0 RC10

AI 이미지·영상 제작자가 생성 전에 인물, 소품, 공간, 카메라, 조명과 움직임을 3D로 연출하고, 결과를 생성 AI용 Shot Package로 출력하는 로컬 우선 프리비주얼라이제이션 도구입니다.

## RC10: 실제 배포본 안정화

RC10은 새 기능을 넓히지 않고, 사용자가 샷을 출력하는 순간과 일반 노트북 화면에서 발생하는 마찰을 줄이는 안정화 버전입니다.

- Shot Package 생성 전 카메라·행동 충돌·끊어진 참조·3D 렌더 가능 여부 사전점검
- 프레임·Pose·Depth·Mask·프롬프트·Manifest 생성 목록 미리보기
- 차단 문제 발생 시 카메라·타임라인·프로젝트 점검으로 바로 이동
- 1366×768 노트북 전용 Chromium 스모크 검사
- 헤더 수평 넘침, 문서 수평 스크롤, 핵심 편집 영역 가시성 자동 검사
- 낮은 화면에서 샷·타임라인·명령창이 밀려나는 레이아웃 문제 수정
- WebGL 안전 모드의 중복 경고 제거
- 프로젝트 스키마 `1.0.0-rc.10`
...
- Tauri 정적 구성
- Chromium 앱 셸·명령 검색·Shot Package 사전점검 상호작용
- 1440×920 기본 화면과 1366×768 노트북 레이아웃
- GLB 리그 Fixture 3종
- 반복 편집·복구 스트레스
- 번들 크기 예산
- 릴리스 증거 무결성 게이트
