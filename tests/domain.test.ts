...
test('0.5 프로젝트를 최신 스키마로 변환한다', () => {
  ...
  assert.equal(result.project?.schemaVersion, '1.0.0-rc.11');
...
test('Shot Package Manifest는 시작·종료 상태와 카메라를 포함한다', () => {
  ...
  assert.equal(manifest.schemaVersion, '1.0.0-rc.11');
...
test('프로젝트 번들은 project.json과 로컬 GLB를 함께 저장하고 복원한다', async () => {
  ...
  assert.equal(imported.project.schemaVersion, '1.0.0-rc.11');
...
test('대표 베타 시나리오는 자연어 Scene부터 Action·Manifest·프로젝트 번들 복원까지 통과한다', async () => {
  ...
  project.schemaVersion = '1.0.0-rc.11';
...
test('1.0 RC 프로젝트 스키마와 데스크톱 패키지 버전이 일치한다', () => {
  ...
  assert.equal(packageJson.version, '1.0.0-rc.11');
...
test('지원 번들은 진단·복구 요약·시각 스냅샷을 하나의 ZIP으로 만든다', async () => {
  ...
  assert.equal(manifest.appVersion, '1.0.0-rc.11');
...
test('지원 번들의 구조 프로젝트는 프롬프트·로컬 키·서버 주소를 제거한다', async () => {
  ...
  const report = await analyzeProjectHealth(project);
  const bundle = await createSupportBundle({ project, report, appVersion: '1.0.0-rc.11' });
  const files = await readStoredZip(bundle);
...
test('동일한 버전과 릴리스 실행 ID를 가진 3개 OS 증거만 무결성 검사를 통과한다', () => {
  ...
  const evidence = (platform: 'linux' | 'windows' | 'macos') => ({
    browser: {
      status: 'pass', platform, version: '1.0.0-rc.11', releaseId: 'release-test-001', generatedAt: '2026-07-13T00:00:00.000Z',
      interaction: { commandPaletteOpen: true, commandInputFocused: true, commandCount: 21, buttonCount: 90 },
    },
    artifacts: {
      status: 'pass', platform, version: '1.0.0-rc.11', releaseId: 'release-test-001', generatedAt: '2026-07-13T00:00:01.000Z',
      artifacts: [{ path: `bundle/${platform}/installer`, bytes: 1024, sha256: sha }],
    },
    runtime: {
      status: 'pass', platform, version: '1.0.0-rc.11', appVersion: '1.0.0-rc.11', releaseId: 'release-test-001', generatedAt: '2026-07-13T00:00:02.000Z',
      webviewLoaded: true, reactReady: true, exitCode: 0, executableBytes: 2048, executableSha256: sha,
    },
  });
  const result = validateReleaseEvidenceMatrix({ linux: evidence('linux'), windows: evidence('windows'), macos: evidence('macos') }, '1.0.0-rc.11', 'release-test-001');
...
test('다른 CI 실행의 증거·잘못된 체크섬·React 미준비 보고서는 릴리스 증거에서 차단한다', () => {
  const result = validatePlatformReleaseEvidence('linux', {
    browser: {
      status: 'pass', platform: 'linux', version: '1.0.0-rc.11', releaseId: 'release-a', generatedAt: '2026-07-13T00:00:00.000Z',
      interaction: { commandPaletteOpen: true, commandInputFocused: true, commandCount: 21, buttonCount: 90 },
    },
    artifacts: {
      status: 'pass', platform: 'linux', version: '1.0.0-rc.11', releaseId: 'release-b', generatedAt: '2026-07-13T00:00:01.000Z',
      artifacts: [{ path: 'bundle/app.deb', bytes: 1024, sha256: 'not-a-sha' }],
    },
    runtime: {
      status: 'pass', platform: 'linux', version: '1.0.0-rc.11', appVersion: '1.0.0-rc.8', releaseId: 'release-a', generatedAt: '2026-07-13T00:00:02.000Z',
      webviewLoaded: true, reactReady: false, exitCode: 2, executableBytes: 2048, executableSha256: 'b'.repeat(64),
    },
  }, '1.0.0-rc.11', 'release-a');
...
test('RC11 릴리스 파이프라인은 증거 ID·체크섬 매니페스트와 1.0 승격 차단 도구를 포함한다', () => {
  const workflow = readFileSync(new URL('../.github/workflows/desktop-build.yml', import.meta.url), 'utf8');
  const gate = readFileSync(new URL('../scripts/release-gate.mjs', import.meta.url), 'utf8');
